    ...
    if (empty($errors)) {
        ...
        if ($user && $user['lockout_until'] && strtotime($user['lockout_until']) > time()) {
            ...
            if ($user) {
                ...
                // Show forgot-password transiently when threshold reached
                $_SESSION['show_forgot_password'] = false; // never show for non-existent accounts
                $_SESSION['auth_error'] = 'Account does not exist.';
            }
        }
    }
...
if (!empty($auth_error)) { 
    ...
    $show_forgot_password = false; // suppress forgot-password when account doesn't exist
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
            ...
            <section class="auth-section">
                <div class="auth-container">
                    ...
                    <?php if (!empty($auth_error)): ?>
                        <div class="error-text" id="auth-error" style="display: block; text-align:center; font-weight:bold;">
                            <?php echo htmlspecialchars($auth_error); ?>
                        </div>
                    <?php endif; ?>
                    
                    <div class="error-text" id="form-error" data-lockout-seconds="<?php echo $lockout_seconds; ?>" style="<?php echo $is_locked_out ? 'display:block; text-align:center; font-weight:bold;' : 'display:none;'; ?>">
                    ...
                    <form id="login-form" class="elegant-form" action="login.php" method="post" novalidate>
                        <div class="form-section">
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="username" name="username" minlength="3" maxlength="80">
                        ...
                        <div class="form-actions">
                            <button type="submit" id="login-button" class="btn primary-btn">Login</button>
                            <p class="form-note">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (usernameInput) {
                ['input','paste'].forEach(evt => usernameInput.addEventListener(evt, () => {
                    enforceMaxLength(usernameInput);
                    const v = usernameInput.value.trim();
                    const min = parseInt(usernameInput.getAttribute('minlength')) || 0;
                    const max = parseInt(usernameInput.getAttribute('maxlength')) || Infinity;
                    if (v.length === 0) {
                        setError(usernameInput, usernameError, 'Username or ID Number is required.');
                    } else if (v.length < min) {
                        setError(usernameInput, usernameError, `Minimum ${min} characters required.`);
                    } else if (v.length > max) {
                        setError(usernameInput, usernameError, `Maximum ${max} characters allowed.`);
                    } else {
            ...
            if (passwordEl) {
                ['input','paste'].forEach(evt => passwordEl.addEventListener(evt, () => {
                    enforceMaxLength(passwordEl);
                    const v = passwordEl.value;
                    const min = parseInt(passwordEl.getAttribute('minlength')) || 0;
                    const max = parseInt(passwordEl.getAttribute('maxlength')) || Infinity;
                    if (v.length === 0) {
                        setError(passwordEl, passwordError, 'Password is required.');
                    } else if (v.length < min) {
                        setError(passwordEl, passwordError, `Password must be at least ${min} characters.`);
                    } else if (v.length > max) {
                        setError(passwordEl, passwordError, `Password must be at most ${max} characters.`);
                    } else {
                        setError(passwordEl, passwordError, '');
                    }
                }));
            }
            ...
            if (loginForm) {
                loginForm.addEventListener('submit', function(e) {
                    ...
                    if (!usernameInput || !usernameInput.value.trim()) {
                        ...
                        const min = parseInt(usernameInput.getAttribute('minlength')) || 0;
                        const max = parseInt(usernameInput.getAttribute('maxlength')) || Infinity;
                        if (v.length < min) { isValid = false; setError(usernameInput, usernameError, `Minimum ${min} characters required.`); }
                        else if (v.length > max) { isValid = false; setError(usernameInput, usernameError, `Maximum ${max} characters allowed.`); }
                        else { setError(usernameInput, usernameError, ''); }
                    }
                    ...
                    if (!passwordEl || !passwordEl.value.trim()) {
                        ...
                        const min = parseInt(passwordEl.getAttribute('minlength')) || 0;
                        const max = parseInt(passwordEl.getAttribute('maxlength')) || Infinity;
                        if (v.length < min) { isValid = false; setError(passwordEl, passwordError, `Password must be at least ${min} characters.`); }
                        else if (v.length > max) { isValid = false; setError(passwordEl, passwordError, `Password must be at most ${max} characters.`); }
                        else { setError(passwordEl, passwordError, ''); }
                    }
